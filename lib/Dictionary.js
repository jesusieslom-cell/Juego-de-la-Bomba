...
class Dictionary {
  ...
  load() {
    ...
    const customPath = path.join(__dirname, '..', 'data', 'custom_words.txt');
    ...
    if (fs.existsSync(dictPath)) {
      ...
      console.log(`📖 Diccionario principal cargado: ${count} palabras desde ${dictPath}`);
    } else {
      console.warn('\n⚠══════════════════════════════════════════════════════════════⚠');
    ...
    if (fs.existsSync(customPath)) {
      ...
      const customLines = customContent.split(/\r?\n/);
      let customCount = 0;
      ...
      console.log(`📖 Palabras custom cargadas: ${customCount} desde ${customPath}`);
    }
  }
  ...
  has(word) {
    ...
    // Plural fallback (conservative)
    if (word.endsWith('s')) {
      // Try removing 's' (e.g. casa -> casas)
      if (this.words.has(word.slice(0, -1))) return true;
      
      // Try removing 'es' (e.g. reloj -> relojes, papel -> papeles)
      if (word.endsWith('es')) {
         if (this.words.has(word.slice(0, -2))) return true;
         
         // Special case for 'z' to 'c' (e.g. luz -> luces)
         if (word.endsWith('ces')) {
             const singularZ = word.slice(0, -3) + 'z';
             if (this.words.has(singularZ)) return true;
         }
      }
    }
    return false;
